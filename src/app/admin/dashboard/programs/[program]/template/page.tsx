...
import { ArrowRight, UploadCloud, LoaderCircleIcon } from 'lucide-react';
...
export default function TemplatePage() {
  ...
  const [template, setTemplate] = useState<Template>({ schemas: [[]], basePdf: '' });
  ...
  const handleBasePdfUpload = async () => {
    if (basePdf) {
      try {
        ...
        setMessage(`Error uploading Base PDF: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  };
  ...
  const handleTemplateSave = async (newTemplate: Template) => {
    try {
      ...
      setMessage(`Error saving template: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
  ...
  return (
    <div className="text-white p-6 flex flex-col items-center">
      ...
      <div className="bg-gray-900 p-8 rounded-lg shadow-lg w-full max-w-4xl">
        <div className="mb-4">
          <label className="block text-white mb-2" htmlFor="basePdf">Upload Base PDF</label>
          <input
        ...
        <div className="mb-4">
          <label className="block text-white mb-2" htmlFor="csvFile">Upload CSV</label>
          <input
            className="w-full p-2 rounded bg-gray-700 text-white file:bg-indigo-700 file:text-white file:font-bold file:px-4 file:py-2 file:rounded file:cursor-pointer file:border-0"
            type="file"
            id="csvFile"
        ...
        <div className="mt-6 w-full">
          <TemplateEditor template={template} onSubmit={handleTemplateSave} />
        </div>
      </div>
    </div>
  );
