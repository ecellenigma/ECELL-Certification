...
import { getParticipants, setParticipant, deleteParticipant, addParticipant } from "@/lib/firebase/firestore";
...
export default function ProgramParticipants() {
  ...
  const [newParticipant, setNewParticipant] = useState<Participant>({ id: '', name: '', rank: 0 });
  ...
  useEffect(() => {
    async function fetchParticipants() {
      if (program) {
        const participantsRef = await getParticipants(program as string) as Participant[];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedParticipant) {
      await setParticipant(program as string, selectedParticipant.id, selectedParticipant);
  ...
  const handleDeleteSelected = async () => {
    const deletePromises = Array.from(selectedParticipants).map(participantId => deleteParticipant(program as string, participantId));
    ...
    <div className="text-white p-6 flex flex-col items-center">
          ...
          <tbody>
            {participants.map(participant => (
              <tr key={participant.id} className="text-center">
                ...
                <td className="py-2 px-4 border-b border-gray-700">{participant.id}</td>
                <td className="py-2 px-4 border-b border-gray-700">{participant.name}</td>
                <td className="py-2 px-4 border-b border-gray-700">{participant.rank}</td>
      ...
      {selectedParticipant && (
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg mt-6 w-full max-w-md">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-4">
              <label className="block text-white mb-2" htmlFor="rank">Rank</label>
              <input
                className="w-full p-2 rounded bg-gray-700 text-white"
                type="text"
                id="rank"
      ...
      {isAddingParticipant && (
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg mt-6 w-full max-w-md">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <label className="block text-white mb-2" htmlFor="id">ID</label>
              <input
                className="w-full p-2 rounded bg-gray-700 text-white"
                type="text"
                id="id"
